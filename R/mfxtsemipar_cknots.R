...
# ==============================================================================
# Knot placement: center as a fixed knot; remaining split L/R by sample mass
# ==============================================================================
#' Generate interior knots with center fixed as one knot
#'
#' Places \code{center} as one of the \code{nknots} knots, then allocates the
#' remaining \code{nknots - 1} knots to the left and right of \code{center}
#' in proportion to the number of observations on each side. Within each side,
#' knots are placed by quantiles (default) or equally spaced if
#' \code{eqspace = TRUE}.
#'
#' Example: \code{nknots = 10}, \code{center = 20}, left share 40\% and right
#' share 60\% yields about 4 left knots, the center knot, and 5 right knots.
#'
#' @param x numeric vector (typically \code{uvar}).
#' @param nknots integer total number of interior knots (including \code{center}).
#' @param center numeric scalar; also used as an interior knot location.
#' @param eqspace logical; equally spaced knots within each side.
#' @param startp optional lower bound for the placement range.
#' @param endp optional upper bound for the placement range.
#' @return Numeric vector of sorted knot locations of length \code{nknots}.
#' @export
gennknots_center <- function(x, nknots, center, eqspace = FALSE,
                             startp = NULL, endp = NULL) {
  x <- x[!is.na(x)]
  if (length(x) == 0L) {
    stop("No non-missing observations available to form knots.")
  }
  if (is.null(center) || length(center) != 1L || !is.numeric(center) ||
      !is.finite(center)) {
    stop("center must be a finite numeric scalar.")
  }
  nknots <- as.integer(nknots)
  if (length(nknots) != 1L || !is.finite(nknots) || nknots < 1L) {
    stop("nknots must be an integer >= 1 (at least the center knot).")
  }
  ...
  if (center <= lo || center >= hi) {
    stop("center must lie strictly inside the knot range (", lo, ", ", hi, ").")
  }
  ...
  x_in <- x[keep]
  x_left <- x_in[x_in < center]
  x_right <- x_in[x_in > center]
  n_left <- length(x_left)
  n_right <- length(x_right)
  n_side <- n_left + n_right
  if (n_side == 0L) {
    stop("No observations strictly left or right of center to allocate knots.")
  }

  n_remaining <- nknots - 1L
  if (n_remaining == 0L) {
    return(center)
  }

  p_left <- n_left / n_side
  n_left_k <- as.integer(round(n_remaining * p_left))
  n_left_k <- max(0L, min(n_remaining, n_left_k))
  if (n_left == 0L) n_left_k <- 0L
  if (n_right == 0L) n_left_k <- n_remaining
  n_right_k <- n_remaining - n_left_k

  place_side <- function(xs, n_k, side_lo, side_hi, side_lab) {
    if (n_k == 0L) return(numeric(0))
    if (!(side_hi > side_lo)) {
      stop("Degenerate ", side_lab, "-of-center range for knot placement.")
    }
    if (eqspace) {
      step <- (side_hi - side_lo) / (n_k + 1L)
      return(as.numeric(seq(side_lo + step, side_hi - step, length.out = n_k)))
    }
    if (length(xs) < n_k + 1L) {
      stop("Not enough observations ", side_lab,
           " of center to form ", n_k, " knots.")
    }
    probs <- seq_len(n_k) / (n_k + 1L)
    as.numeric(stats::quantile(xs, probs = probs, type = 2, names = FALSE))
  }

  left_knots <- place_side(x_left, n_left_k, lo, center, "left")
  right_knots <- place_side(x_right, n_right_k, center, hi, "right")
  knots <- sort(c(left_knots, center, right_knots))
  attr(knots, "n_left") <- n_left_k
  attr(knots, "n_right") <- n_right_k
  attr(knots, "share_left") <- p_left
  attr(knots, "share_right") <- 1 - p_left
  knots
}


#' Ensure center is present among user-supplied knots
ensure_center_knot <- function(knots, center) {
  knots <- as.numeric(knots)
  if (any(!is.finite(knots))) {
    stop("knots must be finite numeric values.")
  }
  if (is.null(center) || length(center) != 1L || !is.numeric(center) ||
      !is.finite(center)) {
    stop("center must be a finite numeric scalar.")
  }
  if (!any(abs(knots - center) <= sqrt(.Machine$double.eps))) {
    knots <- c(knots, center)
  }
  sort(unique(knots))
}


#' Mixed-frequency semiparametric regression with center-anchored knots
#'
#' Variant of \code{mfxtsemipar} in which \code{center} is always included as an
#' interior knot. When knots are generated from \code{nknots}, the remaining
#' \code{nknots - 1} knots are split to the left and right of \code{center} in
#' proportion to the observation mass on each side (see
#' \code{\link{gennknots_center}}). Does not modify the original
#' \code{mfxtsemipar} implementation.
#'
...
#'   \code{bknots}.
#' @param startp numeric; optional lower bound of the interior knot range.
#' @param endp numeric; optional upper bound of the interior knot range.
#' @param degree polynomial degree.
#' @param knots numeric vector of interior knot locations. If supplied and
#'   \code{center} is missing from the vector, \code{center} is inserted.
...
#' @param eqspace use equally spaced knots within each side of \code{center}
#'   when generating from \code{nknots}.
#' @param nknots integer; total number of interior knots including
#'   \code{center} (alternative to \code{knots}).
#' @param center centering value for the spline basis; also forced to be an
#'   interior knot. Defaults to \code{0} if \code{NULL}.
...
#'
#' @return A list of class \code{c("mfxtsemipar_cknots", "mfxtsemipar")},
#'   including \code{rmse} and knot-allocation metadata
#'   (\code{n_left}, \code{n_right}, \code{share_left}, \code{share_right}).
#'   When \code{ucb = TRUE}, \code{fitted} contains additional \code{<gen>_lb}
#'   and \code{<gen>_ub} columns. The reported standard errors and UCB are
#'   justified under undersmoothing; they do not account for smoothing bias.
#'
#' @export
mfxtsemipar_cknots <- function(hf,
                        lf,
                        y,
                        x = NULL,
                        uvar,
  ...
  class(result) <- c("mfxtsemipar_cknots", "mfxtsemipar", "list")
...
#' @export
print.mfxtsemipar_cknots <- function(x, ...) {
  cat("Mixed-frequency semiparametric regression (center-anchored knots)\n")
