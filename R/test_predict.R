...
lf <- merge(lf, unique(hf[,.(id,t,x1_tot,x2_tot,e_tot)]), by=c("id","t"))
lf[, y := x1_tot - x2_tot + d + e_tot]
hf <- hf[,.(id,t,day,x1,x2,x3,gf)]
lf <- lf[,.(id,t,y,x2_tot,d)]
setnames(lf, "x2_tot", "x2")

res <- mfxtsemipar_cv(
  hf=hf, lf=lf, y="y", x="x2", uvar="x3", id="id", tl="t", gen="fitted",
  type="poly", degree=1, center=0, maxnk=4, minnk=2, nfold=5, seed=123,
...
# Predict on a vector
pred_vec <- predict(res, newdata = c(-1, 0, 1))
print(pred_vec)

cat("predict test completed.\n")
