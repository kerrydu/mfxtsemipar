...
gennknots <- function(x, nknots, eqspace = FALSE,
  ...
  if (eqspace) {
    ...
    # match Stata's _pctile, nq(nknots+1): interior quantiles at i/(nknots+1)
    probs <- seq_len(nknots) / (nknots + 1L)
    cuts <- as.numeric(stats::quantile(x[keep], probs = probs, type = 2, names = FALSE))
  }
