*! true random effects regression with semiparametric transformation and cross-validation for optimal knot selection
...
cap program drop mfxtsemipar2_cv
program define mfxtsemipar2_cv, eclass
...
forv i=`minnk'/`maxnk'{
	...
	qui rmse_cv `ydep' `allbins' `varlist0' `weightexp',  cv(`cv') partialout(`partialout1') absorb(`absorb') `iabsorb' id(`id')
	local rmse = r(rmse)
	mat mse = mse \ `rmse'
...
local splinecmd `gensplines' `xvar', gen(__Spline_) knots(`knots') bknots(`bknots') degree(`degree') centerv(`center') `intercept' type(`type') 
 ...
 local cvars 
    foreach v in `varlist0' `partialout1' `allbins' {
            qui by `id' : gen double c_`v' = mean(`v') 
            local cvars `cvars' c_`v'
    }

 if `brep'==0{
    tempvar res0
    reghdfe `ydep' `varlist0' `partialout1' `allbins' `cvars' `weightexp', ///
    ...
    tempvar res0
    qui reghdfe `ydep' `varlist0' `partialout1' `allbins' `cvars' `weightexp', absorb(`absorb') residuals(`res0')
    mat b = e(b)
    ...
    forv b=1/`brep'{
        ...
        qui reghdfe `ystar' `varlist0' `partialout1' `allbins' `cvars' `weightexp', absorb(`absorb')
        mat bi = e(b)
    ...
    // 将V ereturn 为 e(V)
    tempvar res_yhat
    qui reghdfe `ydep' `varlist0' `partialout1' `allbins' `cvars' `weightexp', absorb(`absorb') residuals(`res_yhat')
    mat b = e(b)
    if `"`predy'"'!=""{
        qui predict `predy', `xbd'
        tempfile predy_file
...
program define rmse_cv, rclass
    version 16.0
    syntax varlist [fw aw pw/], id(varname) [CV(varname) n(integer 10) SEED(integer 1234)  opt(string) cluster(varname) LOGLik  absorb(string)  PARTIALOUT(varlist) IABSORB ]
    ...
    local cavrs
    local j = 0
    foreach v in `varlist' `partialout' {
         local j = `j' +1
         tempvar ms`j'
         qui bys `id' : gen double `ms`j'' = mean(`v') 
         local cvars `cvars' `ms`j''
    }
    ...
    forv i=1/`numgid'{
        qui reghdfe `depvar' `varlist' `cvars' `partialout' `weightexp' if `gid'!=`i', a(`absorb',save)
        //mat b = e(b)
        //// tricky (MY-Mg(X))'(MY-Mg(X))
        // reg Y-g(X) on M
        qui predict `gjhat' , xb

        tempvar fes 
        qui gen double `fes' = 0

        if "`iabsorb'"!=""{
            local absorb0 `absorb'
            qui ds __hdfe*__
            local hdfevars `r(varlist)'
            foreach hdfevar in `hdfevars' {
                gettoken fe1 absorb0 : absorb0
                local fe1 = subinstr("`fe1'","#"," ",.)
                // 将hdfevar的估计值扩散到验证集上
                qui bys `fe1' : fillmising `hdfevar', with(any)
                qui replace `fes' = `fes' + `hdfevar'
            }

        }
        qui replace `gjhat' = `depvar' - `gjhat' - `fes' if `gid'==`i'
        qui replace `resi2' = (`gjhat')^2  if `gid'==`i'
        cap drop `gjhat'
        cap drop __hdfe*__
        cap drop `fes'
    }
